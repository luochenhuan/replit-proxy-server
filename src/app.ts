...
import { InMemoryUsageStore } from "./store/usage-store.js";
import { InMemoryLimitStore } from "./store/limit-store.js";
import { InMemoryWindowCounterStore } from "./store/window-counter-store.js";
 ...
 */
export function buildApp(config: Config, deps?: { modelServer?: ModelServer }): FastifyInstance {
  const app = Fastify({
  ...
  const windowStore = new InMemoryWindowCounterStore();
  const limiter = new Limiter(limitStore, usageStore, windowStore);
  const modelServer = deps?.modelServer ?? new ModelServer(config);
  const proxy = new ProxyHandler(modelServer, limiter, usageStore);
  ...
  app.register(async (userScope) => {
    ...
    registerUsageRoutes(userScope, usageStore, limitStore);
  });

  // --- admin surface (separate auth) ---
  app.register(
    async (adminScope) => {
      registerAdminRoutes(adminScope, config, limitStore, usageStore);
    },
    { prefix: "/admin" },
  );
